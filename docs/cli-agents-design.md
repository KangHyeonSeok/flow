...
- flow-core runtime에는 worktree provisioning 컴포넌트가 없다.
  단, flow-cli에는 `GitWorktreeService`(생성·rebase·삭제·머지)와 `RunnerService`(worktree 할당 흐름)가
  이미 존재하므로, 신규 설계보다는 기존 구현을 `IWorktreeProvisioner` 계약으로 추출·재사용하는 방향이 적합하다.
- assignment 생성 시 worktree를 자동 할당하는 로직이 flow-core에는 없다.
...
2\. flow-core runtime에는 worktree provisioning 컴포넌트가 없다.
   `CliDeveloper`는 `Worktree?.Path == null`이면 `TerminalFailure`를 반환하므로 격리 없이 실행되진 않지만,
   flow-core 내에서 worktree를 만들어주는 주체가 없으므로 현재 Developer는 항상 실패한다.
   (flow-cli에는 `GitWorktreeService`와 `RunnerService`에 worktree 생성·정리 흐름이 이미 존재한다.)
...
flow-cli에는 이미 `GitWorktreeService`(생성·rebase·삭제·머지)와 `RunnerService`(worktree 할당)가 있다.
flow-core에서는 이 구현을 직접 참조하지 않고, 계약 인터페이스를 두어 추출·재사용한다.
...
worktree 책임은 RuleEvaluator가 아니라 FlowRunner가 가진다.
`IWorktreeProvisioner`는 runner 의존성으로 두고, assignment 생성 후 runner가 worktree를 부여한다.
`SideEffect.CreateAssignment`는 worktree를 모르는 상태를 유지한다 — RuleEvaluator가 인프라 책임까지 갖는 것을 방지하기 위함이다.
...
flow-core에는 `Assignment.Worktree` 모델만 있고 이를 채우는 런타임이 없다.
flow-cli에는 `GitWorktreeService`(생성·rebase·삭제·머지)가 이미 존재하므로,
이를 `IWorktreeProvisioner` 계약으로 추출·재사용한다.

v1 설계:

- `CliDeveloper`는 worktree가 없으면 실행하지 않는다.
- worktree provisioning은 `IWorktreeProvisioner`가 책임지며, 구현체는 flow-cli의 `GitWorktreeService` 계열을 재사용한다.
- `FlowRunner`가 assignment 생성 후 provisioner를 통해 worktree를 부여한다 (runner 후처리).
- `SideEffect.CreateAssignment`는 worktree를 모르는 상태를 유지한다.
...
4\. `IWorktreeProvisioner` 인터페이스를 생성하고, 구현체는 flow-cli의 `GitWorktreeService` 계열을 추출·재사용한다.
   - `FlowRunner`가 assignment 생성 후 provisioner로 worktree를 부여한다 (runner 후처리 방식).
   - `SideEffect.CreateAssignment`는 worktree를 모르는 상태를 유지한다 — RuleEvaluator에 인프라 책임을 주지 않는다.

5\. `TestValidationRejected` 후 새 Developer assignment에 이전 worktree를 전달한다.
   - side effect payload 확장이 아니라, runner가 이전 assignment 또는 spec context에서 worktree를 복사하는 방식으로 처리한다.

### 8.3 필수 — 프롬프트 보강

6\. Developer/TestValidator 프롬프트에 `evidenceRefs` JSON 응답 예시를 추가한다.
   현재 공통 응답 형식(`PromptBuilder.cs:13`)은 `proposedEvent`, `summary`, `proposedReviewRequest` 중심이고,
   Developer/TestValidator 지시문(`PromptBuilder.cs:175`, `PromptBuilder.cs:203`)은 evidence를 "보고하라"까지만 말한다.
   agent를 실제 활성화하기 전에 응답 형식을 강제해야 파싱이 안정적으로 동작한다.

### 8.4 권장

7\. 프로젝트별 테스트 실행 명령을 설정 파일로 분리한다.
8\. Developer/TestValidator evidence 파일 생성 규칙을 샘플과 함께 고정한다.
...
tools/flow-core.tests/
  ...
  CliAgentRunnerIntegrationTests.cs      # ✅ 통합 경로 테스트 존재
  CliArchitectTests.cs                   # ⬜ 전용 단위 테스트 없음
  CliDeveloperTests.cs                   # ⬜ 전용 단위 테스트 없음
  CliTestValidatorTests.cs               # ⬜ 전용 단위 테스트 없음
...
4. `IWorktreeProvisioner` 인터페이스 + 구현체 (flow-cli `GitWorktreeService` 계열 추출·재사용)
   - `FlowRunner`가 assignment 생성 후 provisioner로 worktree 부여 (runner 후처리)
   - `TestValidationRejected` 후 runner가 이전 assignment에서 worktree 복사
5. Developer/TestValidator 프롬프트에 `evidenceRefs` JSON 응답 예시 추가
6. 세 agent 전용 단위 테스트 (Planner 테스트·통합 테스트는 이미 존재)
7. worktree lifecycle 통합 테스트
...
1. flow-core에는 worktree 모델만 있고, provisioning runtime은 flow-cli에만 존재한다.
...
| Agent | MaxRetries | 카운터 |
| --- | --- | --- |
| Architect | 2 | `ArchitectReviewLoopCount` (기존) |
| Developer | 2 | `ImplementationRetryCount` (신규) |
| TestValidator | 2 | `TestValidationRetryCount` (신규) |

`RetryableFailure`가 MaxRetries를 초과하면 `TerminalFailure`로 격상한다.
이 정책은 `RunnerConfig` 또는 agent definition에서 설정 가능하게 둔다.

**terminal failure 이벤트 선택에 대한 제약:**
현재 `TerminalFailure` 시 `Review` 상태 → `SpecValidationFailed`, 그 외 → `CancelRequested`를 사용한다.
Architect/Developer/TestValidator의 실행 실패(backend crash, timeout 등)를
`ArchitectReviewRejected`나 `TestValidationRejected` 같은 도메인 판단 이벤트로 매핑하는 것은
"실행 실패"를 "도메인 판단"으로 위장하는 셈이므로 적절하지 않다.
별도 execution failure 정책 또는 신규 이벤트가 필요하며, 구체적 이벤트 추가는 후속 설계로 분리한다.
